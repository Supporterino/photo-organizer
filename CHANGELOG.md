...
- Removed default move behaviour and added error exit on non defined method

### Added

- Option to move files to target directory with `--move`
...
- Option to run tool in dry run mode to see changes with `--dry-run`
- Option to extract dates from EXIF data if present instead of file creation with `--exif`
