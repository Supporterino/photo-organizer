    ...
    if recursive:
        ...
        with os.scandir(source) as entries:
            for entry in entries:
                if entry.is_file() and (not file_endings or entry.name.lower().endswith(tuple(file_endings))):
...
def main():
    ...
    for file_path in files:
        ...
        if args.daily:
            target_folder = os.path.join(args.target, str(year), f"{month:02d}", f"{day:02d}")
        else:
