...
def setup_test_directory():
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        test_files = [
            "file1.txt", "file2.log", "image.jpg", "photo.png",
            "document.pdf", "notes.TXT", "script.PY", "archive.zip"
        ]
...
def test_list_files_non_recursive(setup_test_directory):
    ...
    assert len(files) == 8  # Only top-level files
...
def test_list_files_recursive(setup_test_directory):
    ...
    assert len(files) == 9  # Includes nested.txt
...
def test_list_files_exclude_pattern(setup_test_directory):
    """Test excluding files using a regex pattern."""
    temp_dir = setup_test_directory
    files = list_files(temp_dir, exclude_pattern=r".*\.log")
    
    assert len(files) == 7  # Excludes file2.log
...
def test_list_files_combined_filters(setup_test_directory):
    ...
    temp_dir = setup_test_directory
    files = list_files(temp_dir, file_endings=[".txt", ".log"], exclude_pattern=r"file2.*")
